...
async function handleAreaMapClick(e){
  ...
  store.assets.push({id:(crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`),refId,type,name:name.trim()||defaultName,lat:e.latlng.lat,lon:e.latlng.lng}); saveAreaStore(propertyId,store);
  areaSelectedAssetType=null; document.querySelectorAll("[data-asset-type]").forEach(b=>b.classList.remove("active")); if(map) map.getContainer().style.cursor="grab"; renderAreaMap(); $("areaPlaceMessage").textContent=`${name.trim()||defaultName} placed.`;
}
...
function toggleAreaBoundary(){
  ...
  if(!areaBoundaryMode){
    ...
    const store=getAreaStore(propertyId);
    store.boundary=[...areaBoundaryPoints];
    saveAreaStore(propertyId,store);
    areaBoundaryPoints=[];
    map.getContainer().style.cursor='grab';
    renderAreaMap();
    $("areaPlaceMessage").textContent="Property boundary saved on this device.";
  }
}
