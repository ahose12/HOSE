...
async function fetchNearbyHotels(lat, lon, radiusMeters = 24000) {
...
function renderHotels(hotels, center) {
  ...
  if (!sorted.length) {
    $("hotelResults").innerHTML = '<p class="muted">No hotel markers were returned within about 15 miles. Try the selected area in a maps app for additional lodging.</p>';
    return;
  }
...
async function updatePublicLandMap() {
  ...
  try {
    ...
    $("publicHuntMapStatus").textContent = "Showing the selected public hunting area and lodging found within about 15 miles.";

    let hotels = [];
    try { hotels = await fetchNearbyHotels(center.lat, center.lon); }
    catch (hotelError) {
