...
function profileName(p){return p?.display_name||p?.username||"DIE Hunter"}
function socialPersonHtml(p,actions=""){
  return `<div class="social-person"><div class="social-avatar">${esc(socialInitial(profileName(p)))}</div><div class="social-person-main"><strong>${esc(profileName(p))}</strong><small>${p?.username?`@${esc(p.username)}`:"DIE hunter"}</small></div>${actions?`<div class="social-person-actions">${actions}</div>`:""}</div>`;
}
...
function normalizeBuckScore(profile){
  ...
  // Accept all score locations that prior DIE/OpenAI versions may have used.
  const candidates=[
    profile.score,
    profile.estimated_score,
    profile.boone_crockett_score,
    profile.bc_score,
    profile.ai_score,
    profile.ai_traits?.score,
    profile.ai_traits?.estimated_score,
    profile.ai_traits?.boone_crockett_score,
    profile.ai_traits?.gross_score,
    profile.ai_traits?.antler_score,
    profile.ai_traits?.score_estimate,
    profile.metadata?.score,
    profile.metadata?.estimated_score
  ];
  ...
  // If no direct score exists, derive a conservative visual estimate from available antler traits.
  const t=profile.ai_traits||{};
  const points=Number(t.point_count ?? t.points ?? profile.antler_points ?? 0);
  const spread=Number(t.inside_spread ?? t.spread ?? 0);
  const beams=Number(t.main_beam_length ?? t.beam_length ?? 0);
  const mass=Number(t.mass_estimate ?? t.mass ?? 0);
  const tine=Number(t.total_tine_length ?? t.tine_length ?? 0);

  // Only derive when there is at least some antler information.
  if(points || spread || beams || mass || tine){
    let est=0;
    if(spread) est+=spread;
    if(beams) est+=beams*2;
    if(tine) est+=tine;
    if(mass) est+=mass;
    if(!beams && !tine){
      // fallback based on point count when detailed measurements are absent
      est += points ? (points*10 + 20) : 0;
      if(spread) est += 0;
    }
    if(est>0) return Math.round(est*10)/10;
  }
...
function buckScoreDisplay(profile){
  ...
  if(score!=null) return {value:`~${Number(score).toFixed(1)}"`, missing:false};

  // Every indexed buck gets an explicit score state rather than a blank field.
  // "Pending" means DIE has indexed the buck but has not yet produced a defensible estimate.
  return {value:"Pending estimate", missing:true};
}
